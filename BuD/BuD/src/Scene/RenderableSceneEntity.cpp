...
namespace BuD
{
    ...
    RenderableSceneEntity RenderableSceneEntity::Cube(ID3D11Device* device)
    {
		...
		auto res = RenderableSceneEntity();
		res.m_vertexBuffer = std::make_shared<DX11VertexBuffer>(device, vertices.size() * sizeof(Vector2), elements, vertices.data());
		res.m_indexBuffer = std::make_shared<DX11IndexBuffer>(device, DXGI_FORMAT_R16_UINT, indices.size() * sizeof(unsigned short), indices.data());
