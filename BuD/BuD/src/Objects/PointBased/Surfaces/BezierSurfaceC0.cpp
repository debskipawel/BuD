...
namespace BuD
{
			...
			for (int j = 0; j < patchesV; j++)
			{
				for (int i = 0; i < patchesU; i++)
				{
					...
					for (int p = 0; p < 16; p++)
					{
						...
						patchPoints.push_back(points[indexV * (patchesU * 3 + 1) + indexU]);
					}
	...
	BezierSurfaceC0::BezierSurfaceC0(Scene& scene, const std::vector<BezierPatch*>& patches, int patchesU, int patchesV, bool asCylinder)
